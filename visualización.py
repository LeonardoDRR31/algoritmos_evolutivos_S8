...
def calcular_fitness(cromosoma):
    ...
    if max_bajos_en_examen == len(indices_bajos):  # Todos están en el mismo grupo
        penalizacion = 2.0  # Ajusta si quieres mayor impacto

    # Bonus por diversidad de notas (como antes)
    bonus_diversidad = 0
    for examen in ['A', 'B', 'C']:
...
def graficar_resultados(historial, asignaciones, notas):
    ...
    # Histograma de notas por examen
    plt.subplot(1, 3, 2)
    datos = []
    for examen in ['A', 'B', 'C']:
        indices = asignaciones[examen]
        notas_examen = [notas[i] for i in indices]
        for n in notas_examen:
            datos.append({'Examen': examen, 'Nota': n})
    df_notas = pd.DataFrame(datos)
    sns.histplot(data=df_notas, x='Nota', hue='Examen', multiple='stack', palette='Set2', bins=10)
    plt.title("Histograma de Notas por Examen")

    # Boxplot para comparar distribuciones
    plt.subplot(1, 3, 3)
    sns.boxplot(x='Examen', y='Nota', data=df_notas, hue='Examen', palette='Set2', dodge=False, legend=False)
...
# Graficar resultados
graficar_resultados(historial, asignaciones_finales, notas)
