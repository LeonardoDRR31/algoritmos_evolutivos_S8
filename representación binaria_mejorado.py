...
def calcular_fitness(cromosoma):
    ...
    # Penalizar si no hay 13 alumnos por examen
    ...
    for examen in ['A', 'B', 'C']:
        ...
        # Varianza del grupo (penalización)
        varianza = np.var(notas_examen)
        varianzas.append(varianza)
        
        # Diversidad de rendimientos: cantidad de categorías presentes
        # Categorías: bajas (<12), medias (12-16), altas (>16)
        bajas = sum(n < 12 for n in notas_examen)
        medias = sum(12 <= n <= 16 for n in notas_examen)
        altas = sum(n > 16 for n in notas_examen)
        diversidad = sum(x > 0 for x in [bajas, medias, altas])
        diversidades.append(diversidad)
    
    # Penalización total por varianzas (mayor varianza = peor fitness)
    penalizacion_varianza = sum(varianzas)
    
    # Premio total por diversidad (más categorías = mejor fitness)
    premio_diversidad = sum(diversidades)
    
    # Ajustar peso de premio y penalización para balancear
    fitness = -penalizacion_varianza + premio_diversidad * 10
